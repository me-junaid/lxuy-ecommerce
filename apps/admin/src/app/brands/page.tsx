...
function RowMenu({
  ...
  return (
    <div ref={ref} className="relative" onClick={(e) => e.stopPropagation()}>
      <button
        onClick={() => onToggle(isOpen ? "" : (brand._id || ""))}
...
export default function BrandsPage() {
  ...
  const [drawerBrand, setDrawerBrand] = useState<BrandData | null | undefined>(undefined);
  ...
  const loadBrands = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/v1/brands/admin");
  ...
  // Status toggle
  const handleToggleStatus = async (brand: BrandData) => {
    if (!brand._id) return;
    const nextStatus = brand.status === "active" ? "inactive" : "active";
    try {
      await api.patch(`/api/v1/brands/${brand._id}`, { status: nextStatus, isActive: nextStatus === "active" });
      setBrands((prev) =>
        prev.map((b) => (b._id === brand._id ? { ...b, status: nextStatus, isActive: nextStatus === "active" } : b))
      );
      showToast({ message: `Brand "${brand.name}" is now ${nextStatus}.`, type: "success" });
    } catch {
      showToast({ message: "Failed to update brand status.", type: "error" });
    }
  };

  // Archive
  const handleArchive = async (brand: BrandData) => {
    if (!brand._id) return;
    try {
      await api.patch(`/api/v1/brands/${brand._id}`, { status: "archived", isActive: false });
      setBrands((prev) =>
        prev.map((b) => (b._id === brand._id ? { ...b, status: "archived", isActive: false } : b))
      );
      showToast({ message: `Brand "${brand.name}" archived.`, type: "success" });
    } catch {
      showToast({ message: "Failed to archive brand.", type: "error" });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.delete(`/api/v1/brands/${deleteTarget._id}`);
      setBrands((prev) => prev.filter((b) => b._id !== deleteTarget._id));
      showToast({ message: `Brand "${deleteTarget.name}" deleted.`, type: "success" });
      setDeleteTarget(null);
    } catch (err: unknown) {
      const apiErr = err as { data?: { message?: string } };
      showToast({ message: apiErr?.data?.message || "Failed to delete brand.", type: "error" });
    } finally {
      setDeleting(false);
    }
  };
  ...
  const filteredBrands = brands.filter((b) => {
    ...
    const matchesQuery = b.name.toLowerCase().includes(q) || b.slug.toLowerCase().includes(q);
    const matchesStatus = statusFilter === "all" || b.status === statusFilter;
    return matchesQuery && matchesStatus;
  });

  const isDrawerOpen = drawerBrand !== undefined;
    ...
    <AdminLayout>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
            <div>
              ...
              <p className="text-[11px] uppercase tracking-widest text-[#B38F5F] mt-1.5 font-bold">
                Manage the luxury fashion houses and labels in your catalog.
              </p>
            </div>
            <Button
          ...
          {loading ? (
            ...
            <div className="py-32 text-center border border-dashed border-neutral-200 rounded-lg">
              <p className="text-[10px] text-neutral-400 uppercase tracking-[0.2em] font-bold">
                {searchVal ? "No matching brands found" : "No brands added yet"}
              </p>
              {!searchVal && (
                <button
                  onClick={() => setDrawerBrand(null)}
              ...
              <table className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="border-b border-neutral-200/80 text-[9px] uppercase tracking-[0.2em] text-neutral-400 font-bold bg-neutral-50/60">
                    <th className="p-4 pl-5">Brand</th>
                      ...
                      >
                        ...
                        <td className="p-4 pl-5">
                          <div className="flex items-center gap-3.5">
                            {b.logoUrl ? (
                              <div className="w-10 h-10 rounded border border-neutral-200/80 overflow-hidden bg-white shrink-0 p-1 flex items-center justify-center">
                        ...
                        <td className="p-4">
                          <span className="inline-flex items-center px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded bg-neutral-100 text-neutral-700 border border-neutral-200/60">
                            {b.productCount ?? 0} {b.productCount === 1 ? "Product" : "Products"}
                          </span>
                        </td>

                        {/* Status */}
                        <td className="p-4">
                          <span className={`text-[8px] font-bold uppercase tracking-widest border rounded px-2 py-1 ${statusBadge(b.status)}`}>
                            {b.status}
                          </span>
                        </td>
                        ...
                        <td className="p-4 pr-5 text-right">
                          <RowMenu
                            brand={b}
                            isOpen={openMenuId === b._id}
                            onToggle={(id) => setOpenMenuId(id || null)}
                            onEdit={() => setDrawerBrand(b)}
      ...
      <AnimatePresence>
        {isDrawerOpen && (
          <BrandDrawer
            brand={drawerBrand}
            onClose={() => setDrawerBrand(undefined)}
            onSaved={() => {
              loadBrands();
            }}
            showToast={showToast}
                  ...
                  <>
                    ...
                    <div>
                      <h3 className="font-serif text-lg font-light text-[#111111]">
                        Cannot delete assigned brand
                      </h3>
                      <p className="text-xs text-neutral-600 mt-2 leading-relaxed">
                        This brand is currently assigned to <strong className="font-semibold">{deleteTarget.productCount} product{deleteTarget.productCount !== 1 ? "s" : ""}</strong>.
                        To preserve catalog data integrity, please archive the brand or reassign its products first.
                      </p>
                    </div>
                  ...
                  <>
                    <div>
                      ...
                      <p className="text-xs text-neutral-500 mt-1.5 leading-relaxed">
                        This brand has 0 assigned products. Permanent deletion cannot be undone.
                      </p>
                    </div>
                    <div className="flex gap-3 justify-end pt-2">
                      <button
