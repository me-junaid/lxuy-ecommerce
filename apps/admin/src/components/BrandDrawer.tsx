...
interface BrandDrawerProps {
  brand: BrandData | null; // null means create new brand
  onClose: () => void;
  onSaved: (brand: BrandData) => void;
...
export default function BrandDrawer({
  ...
  const isEditing = Boolean(brand && brand._id);
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      showToast({ message: "Brand name is required.", type: "error" });
      return;
    }
    ...
    try {
      ...
      let res;
      if (isEditing && brand?._id) {
        res = await api.patch(`/api/v1/brands/${brand._id}`, payload);
        showToast({ message: `Brand "${name}" updated successfully.`, type: "success" });
      } else {
        res = await api.post("/api/v1/brands", payload);
        showToast({ message: `Brand "${name}" created successfully.`, type: "success" });
      }

      onSaved(res?.data || res || { ...payload, _id: brand?._id });
      setIsDirty(false);
      onClose();
    } catch (err: unknown) {
      ...
      const errMsg = Array.isArray(apiMsg)
        ...
        ? apiMsg
        : apiErr?.message || "Failed to save brand.";
      ...
      >
        ...
        <div className="shrink-0 border-b border-neutral-200/80 px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {logoUrl ? (
              <div className="w-10 h-10 rounded border border-neutral-200 overflow-hidden bg-white shrink-0 p-1 flex items-center justify-center">
            ...
            <div>
              <h2 className="font-serif text-lg font-light text-[#111111] leading-snug">
                {isEditing ? name || "Edit Brand" : "New Brand"}
              </h2>
              <p className="text-[9px] uppercase tracking-wider text-neutral-400 font-bold mt-0.5">
                {isEditing ? "Update brand profile & visibility" : "Add brand to catalog inventory"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            {logoUrl && (
              <div className="mt-2 p-3 border border-neutral-200 rounded bg-white flex items-center gap-3">
                <div className="w-12 h-12 border border-neutral-100 rounded p-1 bg-neutral-50 flex items-center justify-center shrink-0">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={logoUrl} alt="Logo preview" className="max-w-full max-h-full object-contain" />
                </div>
                <div className="text-[10px] text-neutral-400 font-semibold truncate">Logo preview</div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              disabled={saving}
            >
              <option value="active">Active (Visible on storefront & product forms)</option>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="A short editorial bio or history for the brand..."
              disabled={saving}
            />
          </div>

          {isEditing && brand?.productCount !== undefined && (
